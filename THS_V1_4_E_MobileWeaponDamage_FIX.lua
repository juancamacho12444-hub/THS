--//====================================================
--// THS - Tactical Hitbox System V1.4-E
...
--[[
	...
	Esta V1.4-E agrega Damage Engine móvil + arma, conservando hitboxes auxiliares, Color Engine y Debug Engine,
...
THS.Config = {
	...
	DamageEnabled = true,
	DamageTestFireEnabled = true, -- Interno: se deja activo siempre.
	BaseDamage = 25,
	DamageMaxDistance = 500,
	DamageCooldown = 0.15,
	DamageAssistRadius = 4,
	DamageMobileButtonEnabled = true,
	DamageToolActivatedEnabled = true,
...
function THS:SyncDamageSettingsToServer()
	-- En V1.4-E ya no sincronizamos con servidor.
...
function THS:GetShotRay(screenPosition)
	if not Camera then
		return nil, nil
	end

	local maxDistance = self.Config.DamageMaxDistance
	...
	-- En teléfono y armas tipo Tool usamos el centro de la cámara.
	return Camera.CFrame.Position, Camera.CFrame.LookVector * maxDistance
end

function THS:FindBestHitboxByRay(origin, direction, hitboxParts)
	local maxDistance = self.Config.DamageMaxDistance
	...
	for _, part in ipairs(hitboxParts) do
		if part and part.Parent then
			...
			local along = toPart:Dot(unit)

			if along >= 0 and along <= maxDistance then
				local closestPoint = origin + (unit * along)
				local distanceFromRay = (part.Position - closestPoint).Magnitude
				local boxRadius = math.max(part.Size.X, part.Size.Y, part.Size.Z) * 0.5
				local allowedRadius = boxRadius + self.Config.DamageAssistRadius

				if distanceFromRay <= allowedRadius then
					local _, model, group = self:FindHitboxOwner(part)
					if model then
						local score = distanceFromRay + (along * 0.002)
	...
	return bestPart, bestModel, bestGroup
end

function THS:RequestDamageShot(screenPosition)
	if not self.Config.SystemEnabled or not self.Config.DamageEnabled then
		return
	end
	...
	local origin, direction = self:GetShotRay(screenPosition)
	if not origin or not direction then
		self.Runtime.LastDamageResult = "No se pudo leer el disparo"
		return
	end
	...
	if result and result.Instance then
		_, targetModel, partGroup = self:FindHitboxOwner(result.Instance)
	end

	-- Apoyo para teléfono: si el rayo no pegó exacto, busca la hitbox más cercana a la línea de mira.
	if not targetModel then
		local _, fallbackModel, fallbackGroup = self:FindBestHitboxByRay(origin, direction, hitboxParts)
		targetModel = fallbackModel
		partGroup = fallbackGroup
	end
	...
	local ok, damageOrReason = self:ApplyClientDamage(targetModel, partGroup or "Global")
	if ok then
		self.Runtime.DamageHitsConfirmed += 1
		self.Runtime.LastDamageResult = "Hit: " .. tostring(targetModel.Name) .. " | -" .. tostring(math.floor(damageOrReason * 100) / 100)
	else
...
function THS:RefreshPage()
	...
	elseif page == "Daño" then
		...
		self:SetPageInfo(
			"Daño activo por defecto." ..
			"\n\nNo necesitas activar una opción extra." ..
			"\nEn teléfono puedes usar el botón FIRE o disparar con un Tool/arma." ..
			"\nTodas las partes hacen el mismo daño base por ahora." ..
			"\n\nÚltimo resultado: " .. tostring(self.Runtime.LastDamageResult)
		)

		self:CreateNumberControl(options, "Daño base", 0, "BaseDamage", 1, 200, 1, "Daño fijo que se aplica al objetivo.")
		self:CreateNumberControl(options, "Distancia daño", 55, "DamageMaxDistance", 20, 2000, 10, "Distancia máxima del disparo.")
		self:CreateNumberControl(options, "Cooldown daño", 110, "DamageCooldown", 0.05, 2, 0.05, "Tiempo mínimo entre disparos.")
		self:CreateNumberControl(options, "Ayuda móvil", 165, "DamageAssistRadius", 0, 20, 1, "Ayuda para que en teléfono no tenga que pegar exactamente al pixel.")
		self:CreateToggle(options, "Botón FIRE móvil", 220, "DamageMobileButtonEnabled", "Muestra un botón táctil para probar daño en teléfono.")
		self:CreateToggle(options, "Conectar armas Tool", 265, "DamageToolActivatedEnabled", "Cuando un Tool/arma se activa, THS intenta aplicar el daño.")
	...
	if options then
		...
		elseif page == "Daño" then
			options.CanvasSize = UDim2.fromOffset(860, 440)
		elseif page == "Colores" then
...
function THS:CreateUI()
	...
	local fireButton = create("TextButton", {
		Name = "THS_MobileFireButton",
		Size = UDim2.fromOffset(86, 86),
		Position = UDim2.new(1, -115, 1, -135),
		BackgroundColor3 = Theme.Red,
		BackgroundTransparency = 0.08,
		Text = "FIRE",
		TextColor3 = Theme.Text,
		...
		TextSize = 18,
		AutoButtonColor = false,
		Visible = UserInputService.TouchEnabled and self.Config.DamageMobileButtonEnabled,
	}, gui)
	create("UICorner", { CornerRadius = UDim.new(1, 0) }, fireButton)

	connect(fireButton.MouseButton1Click, function()
...
--//====================================================
--// DAMAGE INPUT: MÓVIL + ARMAS TOOL
--//====================================================

function THS:BindTool(tool)
	if not tool or not tool:IsA("Tool") then
		return
	end

	if self.Runtime.ToolConnections[tool] then
		return
	end

	local connection = tool.Activated:Connect(function()
		if self.Config.DamageToolActivatedEnabled then
			self:RequestDamageShot()
		end
	end)

	self.Runtime.ToolConnections[tool] = connection
	table.insert(self.Runtime.Connections, connection)
end

function THS:BindExistingTools()
...
function THS:BindToolAutoDamage()
	...
	if LocalPlayer.Character then
		connect(LocalPlayer.Character.ChildAdded, function(child)
			self:BindTool(child)
		end)
	end
	...
	self:BindExistingTools()
end

function THS:RefreshDamageMobileButton()
	if self.UI and self.UI.MobileFireButton then
		self.UI.MobileFireButton.Visible = UserInputService.TouchEnabled
			and self.Config.DamageMobileButtonEnabled
			and not self.Runtime.IsShuttingDown
	end
end
...
function THS:Update(deltaTime)
	self.Config.DamageEnabled = true
	self:RefreshDamageMobileButton()
...
self.Runtime.LastFPSUpdate = os.clock()
	...
	connect(UserInputService.InputBegan, function(input, gameProcessed)
		...
		elseif input.UserInputType == Enum.UserInputType.Touch then
			-- En móvil el botón FIRE y Tool.Activated son lo principal.
			-- Este toque sirve como respaldo cuando no se usa un Tool.
			self:RequestDamageShot(Vector2.new(input.Position.X, input.Position.Y))
			return
		end
	...
	print("[THS] Tactical Hitbox System V1.4-E Mobile Weapon Damage FIX iniciado.")
end
